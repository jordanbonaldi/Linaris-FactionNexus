...
public class CmdList extends FCommand {
    ...
    public CmdList() {
        ...
        permission = Permission.LIST.node;
        disableOnLock = false;
    ...
    public void perform() {
        ...
        if (!this.payForCommand(Conf.econCostList, "to list the factions", "for listing the factions")) { return; }

        final ArrayList<Faction> factionList = new ArrayList<Faction>(Factions.i.get());
        ...
        Collections.sort(factionList, new Comparator<Faction>() {
            @Override
            public int compare(final Faction f1, final Faction f2) {
                final int f1Size = f1.getFPlayers().size();
                final int f2Size = f2.getFPlayers().size();
                if (f1Size < f2Size) {
                    return 1;
        ...
        Collections.sort(factionList, new Comparator<Faction>() {
            @Override
            public int compare(final Faction f1, final Faction f2) {
                final int f1Size = f1.getFPlayersWhereOnline(true).size();
                final int f2Size = f2.getFPlayersWhereOnline(true).size();
                if (f1Size < f2Size) {
                    return 1;
                } else if (f1Size > f2Size) { return -1; }
                return 0;
        ...
        final int pagecount = factionList.size() / pageheight + 1;
        ...
        for (final Faction faction : factionList.subList(start, end)) {
            if (faction.isNone()) {
                lines.add(p.txt.parse("<i>Factionless<i> %d online", Factions.i.getNone().getFPlayersWhereOnline(true).size()));
                continue;
            }
