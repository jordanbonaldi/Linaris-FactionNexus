...
public class Econ {
    ...
    public static boolean modifyMoney(final EconomyParticipator ep, final double delta, final String toDoThis, final String forDoingThis) {
        ...
        if (delta > 0) {
            ...
            if (Econ.econ.has(acc, -delta) && Econ.econ.withdrawPlayer(acc, -delta).transactionSuccess()) {
    ...
    public static boolean setBalance(final String account, final double amount) {
        ...
        if (current > amount) {
            return Econ.econ.withdrawPlayer(account, current - amount).transactionSuccess();
        } else {
            return Econ.econ.depositPlayer(account, amount - current).transactionSuccess();
        }
    }
