...
public class Worldguard {
    ...
    public static boolean isPVP(final Player player) {
        if (!Worldguard.enabled) { return true; }
    ...
    public static boolean playerCanBuild(final Player player, final Location loc) {
        if (!Worldguard.enabled) { return false; }

        final World world = loc.getWorld();
        final Vector pt = BukkitUtil.toVector(loc);

        if (Worldguard.wg.getRegionManager(world).getApplicableRegions(pt).size() > 0) { return Worldguard.wg.canBuild(player, loc); }
        return false;
    }
