...
public class PermUtil {
    ...
    public String getForbiddenMessage(final String perm) {
        return p.txt.parse(Lang.permForbidden, this.getPermissionDescription(perm));
    }
    ...
    public final void setup() {
        for (final Permission permission : p.getDescription().getPermissions()) {
            //p.log("\""+permission.getName()+"\" = \""+permission.getDescription()+"\"");
            permissionDescriptions.put(permission.getName(), permission.getDescription());
        }
    }

    public String getPermissionDescription(final String perm) {
        final String desc = permissionDescriptions.get(perm);
