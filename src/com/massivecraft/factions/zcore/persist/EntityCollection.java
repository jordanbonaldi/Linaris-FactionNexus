...
public abstract class EntityCollection<E extends Entity> {
    ...
    public boolean exists(final String id) {
        ...
        return this.id2entity.get(id) != null;
    }
    ...
    public boolean saveToDisc() {
        if (this.saveIsRunning) { return true; }
    ...
    private Map<String, E> loadCore() {
        if (!this.file.exists()) { return new HashMap<String, E>(); }
        ...
        try {
            ...
            Bukkit.getLogger().log(Level.WARNING, "JSON error encountered loading \"" + this.file + "\": " + ex.getLocalizedMessage());

            // backup bad file, so user can attempt to recover something from it
            final File backup = new File(this.file.getPath() + "_bad");
            if (backup.exists()) {
                backup.delete();
            }
