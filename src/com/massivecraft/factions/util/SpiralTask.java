...
public abstract class SpiralTask implements Runnable {
    ...
    public SpiralTask(final FLocation fLocation, final int radius) {
        // limit is determined based on spiral leg length for given radius; see insideRadius()
        limit = (radius - 1) * 2;

        world = Bukkit.getWorld(fLocation.getWorldName());
        if (world == null) {
        ...
        x = (int) fLocation.getX();
        z = (int) fLocation.getZ();
    ...
    public final boolean insideRadius() {
        final boolean inside = current < limit;
        if (!inside) {
            this.finish();
        }
        return inside;
